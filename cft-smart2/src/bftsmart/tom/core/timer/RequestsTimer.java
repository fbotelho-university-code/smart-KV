...
public class RequestsTimer {
    ...
    public void run_lc_protocol() {
        ...
        /**
    	 * IGOR CODE
    	 * ISTO SERVE PARA VERIFICAR SE A REPLICA ESTA LENTA OU NÌO; VAMOS VERIFICAR SE ESTOU LENTO OU SE ƒ O LIDER
    	 */
    	/*if(this.tomLayer.getInExec()!= -1){
	    	ExecutionManager execManager = this.tomLayer.execManager;
	    	Execution execActual = execManager.getExecution(this.tomLayer.getInExec());
	    	
	    	Round round = execActual.getRound(0, this.tomLayer.reconfManager);
	    	if(round.propValueHash != null){
	    		if(round.countWeak(round.propValueHash) > this.tomLayer.reconfManager.getQuorumStrong()){
	    			rtTask = new RequestTimerTask();
	                timer.schedule(rtTask, t);
	    			return;
	    		}
