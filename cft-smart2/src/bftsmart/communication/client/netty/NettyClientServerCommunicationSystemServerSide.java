...
public class NettyClientServerCommunicationSystemServerSide extends SimpleChannelHandler implements CommunicationSystemServerSide {
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) {
    	...
    	Logger.println("NETTY MESSAGE FROM: "+((TOMMessage) e.getMessage()).getSender()+" AND ID: "+((TOMMessage) e.getMessage()).getId());
    ...
    public void send(int[] targets, TOMMessage sm, boolean serializeClassHeaders) {
        ...
        for (int i = 0; i < targets.length; i++) {
            ...
            if (ncss != null) {
                ...
                //send message
                sendLock.lock();
                //System.out.println("NETTY:   ESTOU A PRETPARAR PARA ENVIAR: "+sm.getSequence()+" PARA O CLIENTE: "+sm.destination);
                session.write(sm); // This used to invoke "await". Removed to avoid blockage and race condition.
                //System.out.println("NETTY:   ACABEI DE ENVIAR: "+sm.getSequence()+" PARA O CLIENTE: "+sm.destination);
                sendLock.unlock();
            } else {
                rl.readLock().unlock();
            }
