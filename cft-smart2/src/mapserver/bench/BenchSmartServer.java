...
public class BenchSmartServer extends DefaultSingleRecoverable {
	...
	SummaryStatistics throughputInSize;
	final private boolean measure ;
	final private long interval;
	private long executions=0;
	private long message_size = 0;  
	...
	public static void main(String[] args){
		...
		if (args.length ==  2) {
			...
			new BenchSmartServer(id, interval);
		}
		else{
			new BenchSmartServer(id); 
		}
	...
	public BenchSmartServer(int id) {
	...
	public BenchSmartServer(int i, long interval) {
		replica = new ServiceReplica(i, this, this); 
		this.interval = interval; 
		this.measure = true;
		throughput = new DescriptiveStatistics();
		throughputInSize = new SummaryStatistics(); 
	    				...
	    				public void run() {
	    			        ...
	    			        try{
	    			        	...
	    			        	out.println(statsEnd.getN() + ":" + statsEnd.getMean() +  ":" + statsEnd.getStandardDeviation() + ":" + statsEnd.getMin() + ":" +statsEnd.getMax() + 
	    			        		...
	    			        		+ ": " + ordered + ":" + unordered + ":" + throughputInSize.getMean() + ":" + throughputInSize.getStandardDeviation());  
	    			        	out.close();
	    			        	}
	    					} catch (IOException e) {
	...
	private byte[]execute(byte[] command) throws IOException{
		 ...
		 if (warmed){
			 this.message_size += command.length;
		 }
		 ...
		 if (val > 0  ){
			 ...
			 if (measure && warmed){
				 ...
				 if ((executions % interval) == 0 ){
					 executions = 0; 
					 try{
						 long time = (System.currentTimeMillis() - tpStartTime); 
						 throughput.addValue(1000 * interval/ time);
						 //MEasure mbytes/sec 

						 
						 throughputInSize.addValue(1000 * (this.message_size /1024*10224*8) / time );
						 message_size =0; 
